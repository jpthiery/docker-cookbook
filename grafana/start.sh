...
cat /grafana/apache-grafana.conf | sed  -e "s#@@GRAPHITE_HOST@@#${GRAPHITE_PORT_80_TCP_ADDR}:${GRAPHITE_PORT_80_TCP_PORT}/#" \
  -e "s#@@ELS_HOST@@#${ELS_PORT_9200_TCP_ADDR}:${ELS_PORT_9200_TCP_PORT}/#g"\
  > /etc/apache2/sites-available/grafana.conf
sudo a2ensite grafana

cat config.js | sed -e "s#@@GRAPHITE_HOST@@#${IP_EXPOSE}:${PORT_EXPOSE}/graphite/#g" \
  -e "s#@@ELS_HOST@@#${IP_EXPOSE}:${PORT_EXPOSE}/els/#g" \
  > grafana/config.js
